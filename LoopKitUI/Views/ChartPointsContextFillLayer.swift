        ...
        .chartYAxis {
            AxisMarks(position: .leading, values: yAxisValues) { value in
                ...
                AxisValueLabel(horizontalSpacing: settings.labelsToAxisSpacingY) {
                    if let scalar = value.as(Double.self) {
        ...
        GeometryReader { geo in
            ZStack {
                   ...
                   let highlighted = nearestPoint(in: spec, to: touchLocation, proxy: proxy, geo: geo)
                {
                    ...
                    Circle()
                        .fill(Color(spec.tintColor.withAlphaComponent(0.5)))
                        .frame(width: 16, height: 16)
                        .position(highlighted.position)

                    // The value label above the chart
                    let label = highlighted.point.y.label
                    let estimatedHalfWidth = max(20, CGFloat(label.count) * 4.6)
                    let labelX = min(max(highlighted.position.x, plotFrame.minX + estimatedHalfWidth), plotFrame.maxX - estimatedHalfWidth)
                    Text(label)
                        .font(Font(UIFont.monospacedDigitSystemFont(ofSize: 15, weight: .bold)))
                        .foregroundColor(Color(spec.tintColor))
                        .position(x: labelX, y: plotFrame.minY - 21)
                }
            }
